...
// Função genérica para acender LEDs com determinada cor
static void mostrar_numero(const uint8_t indices[], uint tamanho, uint8_t r, uint8_t g, uint8_t b) {
    ...
    for (uint i = 0; i < tamanho; i++) {
        npSetLED(indices[i], r, g, b); // Acende cada LED no índice com a cor dada
    }
    npWrite(); // Atualiza a matriz de LEDs
}

// Mostra o número 1 na matriz
